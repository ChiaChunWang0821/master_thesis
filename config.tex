...
\def\degree    {碩士} % 碩士/博士
\def\titleZh   {基於深度學習之嬰兒危險監測系統}   % Chinese title
\def\titleEn   {A Deep-learning-based Danger Monitoring System \protect\\ for Infants} % English title
...
\def\profs     {蘇木春}
\def\degreedate{中~華~民~國~一百一十一年~六~月}
\def\copyyear  {2020-2022}

\setboolean{printcopyright}{false} % {true}/{false} print copyright text on titlepage or cover.

\def\keywordsZh{嬰兒危險監測, 嬰兒臉部遮擋, 嬰兒姿勢, 深度學習, 嬰兒猝死症}
\def\keywordsEn{infant danger monitoring, infant face occlusion, infant posture, deep learning, sudden infant death syndrome}
