...
\def\deptshort {資訊工程}
\def\dept      {資訊工程學系}%XX學系XXX碩士班(請參考「中央大學學位論文撰寫體例參考」附錄)
\def\degree    {碩士} % 碩士/博士
\def\titleZh   {基於深度學習之冠狀動脈分割及其應用}   % Chinese title
\def\titleEn   {The Development of Deep Learning-based Coronary Artery Segmentation Methods and its Applications} % English title
...
\def\profs     {蘇木春}
\def\degreedate{中~華~民~國~一百一十年~六~月}
\def\copyyear  {2019-2021}

\setboolean{printcopyright}{false} % {true}/{false} print copyright text on titlepage or cover.

\def\keywordsZh{冠狀動脈, 深度學習, 醫學影像, 影像處理, 電腦視覺}
\def\keywordsEn{Coronary Artery, Deep Learning, Medical Image Processing, Image Processing, Computer Vision}
...
% 學術倫理修課證明
\def\letterCareCert {letter_caree_certification.pdf}
% 論文相似度比對報告電子回條
\def\letterThesisSim {letter_thesis_simularity.pdf}
% 遠距口試申請
\def\letterRemoteExam {letter_remote_exam.pdf}
