...
嬰兒骨架圖在俯視角與平視角中多有相似之處（如\cref{fig:fig-posture-skeleton-detection-viewport}），
而無法達到良好的辨識結果。
\begin{figure}[!hbt]
    \centering
    \subcaptionbox
...
\begin{figure}[!hbt]
    ...
    \caption{不同視角之嬰兒骨架偵測結果~\cite{noauthor_pose_nodate}}
...
\begin{enumerate}
    ...
    \item 趴躺：嬰兒腹部面朝下，包含趴著或爬行等多動作，而頭部及四肢位置不限，如\cref{fig:fig-posture-sleep-on-stomach}。
    \item 坐姿：嬰兒臀部貼於水平面，而背部未貼於同一平面，頭部及四肢位置不限，如\cref{fig:fig-posture-sit}。
